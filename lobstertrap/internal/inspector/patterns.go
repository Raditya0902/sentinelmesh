...
var HarmPatterns = compile("harm", []string{
	// Weapons / explosives
	`(?i)\b(how\s+to|instructions?\s+for|guide\s+to|steps?\s+to|teach\s+me)\b.{0,30}\b(make|making|build|building|create|creating|construct|constructing|assemble|assembling|manufacture|manufacturing|synthesize|synthesizing)\b.{0,30}\b(bomb|explosive|IED|grenade|detonator|incendiary)\b`,
	`(?i)\b(how\s+to|instructions?\s+for|guide\s+to)\b.{0,30}\b(make|making|build|building|create|creating|synthesize|synthesizing|manufacture|manufacturing)\b.{0,30}\b(poison|toxin|nerve\s+agent|chemical\s+weapon|biological\s+weapon)\b`,
	`(?i)\b(how\s+to|instructions?\s+for)\b.{0,30}\b(make|making|build|building|modify|modifying|convert|converting)\b.{0,30}\b(gun|firearm|weapon|silencer|suppressor)\b`,
	`(?i)\b(thermite|napalm|ricin|sarin|mustard\s+gas|chlorine\s+gas|ANFO|TATP|RDX|C-?4)\b.{0,30}\b(recipe|synth|make|making|create|creating|produce|producing)\b`,
	`(?i)\b(recipe|synth|make|making|create|creating|produce|producing)\b.{0,30}\b(thermite|napalm|ricin|sarin|mustard\s+gas|chlorine\s+gas|ANFO|TATP|RDX|C-?4)\b`,
	// Harm to people
...
var ObfuscationPatterns = compile("obfuscation", []string{
	...
	// Raw base64 payload — long encoded string submitted without "decode" context words
	`[A-Za-z0-9+/]{50,}={0,2}`,
})
